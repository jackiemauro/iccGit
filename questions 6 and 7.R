...
# merge dummies, pi's and geographies
for.icc.covs<-join_all(list(for.pi,dummy.people,covs.set))
detach(for.pi)
attach(for.icc.covs)
...
uncond.SID.cov <- lmer(pi.jk~1 + ed + marital + working + work.type + 
                     ...
                     victim +  
                     (1|SID))
summary(uncond.SID.cov)
df.SID.cov <- as.data.frame(VarCorr(uncond.SID.cov))
icc.SID.cov <- df.SID.cov[1,4]/(df.SID.cov[1,4]+df.SID.cov[2,4]) #0.2623424
...
lmer.zip.cov<-lmer(pi.jk~1 + ed + marital + working + work.type + 
                 ...
                 victim + 
                 (1|zip))
summary(lmer.zip.cov)
df.zip.cov <- as.data.frame(VarCorr(lmer.zip.cov))
icc.zip.cov <- df.zip.cov[1,4]/(df.zip.cov[1,4]+df.zip.cov[2,4]) #0.01917204
