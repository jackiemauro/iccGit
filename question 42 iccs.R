...
n = 6 #number of items in this question
...
icc.SID <- df.SID[1,4]/(df.SID[1,4]+df.SID[2,4]) #0.08381475

# zip level
lmer.zip<-lmer(pi.jk~1 + (1|zip))
summary(lmer.zip)
df.zip<-as.data.frame(VarCorr(lmer.zip))
icc.zip<-df.zip[1,4]/(df.zip[1,4]+df.zip[2,4]) #0.004499878
...
# 0.09046151   0.08964002   0.05181653   0.14884515   0.03756943 